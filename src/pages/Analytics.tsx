...
import { TrendingUp, Users, ShoppingBag, DollarSign, Brain, Zap } from 'lucide-react';
...
export default function Analytics() {
  ...
  const totalSpending = customers.reduce((sum, customer) => sum + (customer.attributes?.totalSpending || 0), 0);

  // Customer spending distribution
  const spendingData = customers
    .map(c => ({
      name: c.name || c.email.split('@')[0],
      spending: c.attributes?.totalSpending || 0
    }))
